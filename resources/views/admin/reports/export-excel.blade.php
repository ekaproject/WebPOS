...
<body>
    <table border="1">
        <tr><td colspan="6"><strong>Laporan Transaksi</strong></td></tr>
        <tr><td colspan="6">Dibuat: {{ $generatedAt->format('d M Y H:i') }}</td></tr>
        <tr><td colspan="6">Filter Tanggal: {{ $filters['from'] ?: '-' }} s/d {{ $filters['to'] ?: '-' }}</td></tr>
        <tr><td colspan="6">Filter Status: {{ $filters['status'] ?: 'semua' }}</td></tr>
    </table>
        ...
        <tbody>
            @forelse($transactions as $transaction)
                ...
                <tr>
                    ...
                    <td>{{ $transaction->status === 'paid' ? $laba : '-' }}</td>
